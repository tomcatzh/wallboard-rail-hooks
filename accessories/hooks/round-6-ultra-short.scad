// round-6-ultra-short.scad — rail-loaded 28.6 mm-tall Ø6 round-rod J hook
// （仅由轨道承载的 28.6 mm 高、直径 6 mm 圆杆 J 钩）
//
// This is an independent extreme-height variant. The original Round 6 and
// physically printed Round 6 Short remain unchanged for A/B comparison. The
// fit-validated rail claw stays fixed, but this model deliberately disables
// the 5 mm lower wall-contact face and routes its load through the rail alone.
//
// The J keeps the original Ø6 rod path, 25 mm outer envelope, centered
// side-print position, and spherical endpoint. The complete hook/root assembly
// rises by the removed 5 mm contact height. Its new flat bottom is Y=-17.4, and
// the full-width D root uses the complete plate from Y=-8 to that bottom without
// adding an upper bulge. One continuous cubic-Bezier loft contracts the width
// to the unchanged Ø6 circular section only at the lowest load-bearing arc.
...
/* [Rail-only compact root] */
// Start from the physically printed Short datum, then remove exactly the 5 mm
// lower wall-contact height. This model intentionally bears on the rail only.
short_body_back_y = -22.4;
wall_contact_lift = wall_contact_h;
body_back_y = short_body_back_y + wall_contact_lift;
...
// Use the full front plate below Y=-8 without pushing the clean upper contour
// into the fixed rail zone.
root_center_y = -pad_top_drop - root_upper_extent;
...
tip_gap_to_mount_top = y_top - tip_top_y;
...
assert(abs(wall_contact_lift - 5.0) < 0.001,
       "ultra-short lift must equal the removed 5 mm wall-contact height");
...
assert(abs(root_top_y + pad_top_drop) < 0.001,
       "loft root top must meet the fixed Y=-8 body datum");
assert(tip_top_y < y_top,
       "raised round tip reaches above the fixed mount envelope");
...
echo(str("hook-round6-ultra-short: rod = ", rod_d,
         " mm; outer span = ", j_span,
         " mm; J center/bottom = ", j_center_y, "/", j_bottom_y,
         " mm; overall height = ", overall_height, " mm"));
echo(str("hook-round6-ultra-short: original height = ", original_overall_height,
         " mm; reduction = ", height_reduction,
         " mm; mount width = ", part_w));
echo(str("hook-round6-ultra-short: centered side-print support gap = ",
         ...
         "; tip Y/gap to mount top = ", tip_top_y, "/",
         tip_gap_to_mount_top, " mm; wall contact = off"));
