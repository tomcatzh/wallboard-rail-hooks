...
- EN: This calculation is not a load rating. The 0.6 mm positive gusset overlap and tangent R1.5 inner arcs remove face-only contacts and sharp re-entrant corners, but no FEA or physical load test has been performed on the raised reinforced revision. It still requires fit and progressive testing with the actual frame.
  ZH: 该计算不是承载额定值。斜撑 0.6 mm 的实体重叠与相切 R1.5 内弧消除了仅共面接触和尖锐内凹角，但抬高加强版尚未进行 FEA（有限元分析）或实物承载测试，仍须搭配实际相框进行配合与逐级实测。
...
- EN: Open item: print and fit the raised revision, then confirm that the frame hides the mount at the 12 mm datum without rail/frame interference. Continue observing for creep, layer separation, rail unseating, hardboard-edge marking, or visible outward lean; no sustained-load rating is assigned.
  ZH: 开放项：打印并试装抬高版，确认相框能在 12 mm 基准下遮住安装件，且导轨/相框不发生干涉。同时继续观察螺变、层间开裂、导轨脱位、硬质背板边缘压痕或可见外倾；不赋予持续承载额定值。
