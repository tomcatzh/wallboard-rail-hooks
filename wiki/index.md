...
- EN: [hook-round6](outputs/hook-round6.md) — experimental 25 mm-span J hook (`hooks/hook-round6.scad`) with a centered, unthreaded Ø6 mm round rod, spherical tip, broad load-spreading root, and support-aware print notes.
  ZH: [hook-round6](outputs/hook-round6.md) —— 实验性 25 mm 大跨 J 钩（`hooks/hook-round6.scad`），采用居中的无螺纹 Ø6 mm 圆杆、球面圆头、宽根部传力过渡，并记录支撑打印要求。
- EN: [hook-keyhole9](outputs/hook-keyhole9.md) — compact keyhole-plate accessory (`hooks/hook-keyhole9.scad`) with a 34.7 mm vertical envelope, 8 mm projection, Ø9 head, Ø3.8 capture neck, and Ø7 reinforced root.
  ZH: [hook-keyhole9](outputs/hook-keyhole9.md) —— 紧凑 keyhole plate（钥匙孔挂片）配件（`hooks/hook-keyhole9.scad`），竖向包络 34.7 mm、凸出 8 mm，采用 Ø9 头、Ø3.8 捕获颈与 Ø7 加强根部。
