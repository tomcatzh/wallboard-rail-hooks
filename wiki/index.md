...
- EN: [library-architecture](topics/library-architecture.md) — reusable-code boundaries: the fixed rail interface lives in `lib/rail-mount.scad`, the parametric J profile lives in `lib/j-hook.scad`, and `misc/` is deliberately avoided.
  ZH: [library-architecture](topics/library-architecture.md) —— 可复用代码边界：固定轨道接口位于 `lib/rail-mount.scad`，参数化 J 轮廓位于 `lib/j-hook.scad`，并明确不采用 `misc/`。
...
- EN: [hook-scad-v1](outputs/hook-scad-v1.md) — the model's full version history v1→v8; the fixed rail library includes the selected full-width 2.0 mm lower wall-contact face, while the J-body generator now lives in its own helper library.
  ZH: [hook-scad-v1](outputs/hook-scad-v1.md) —— 模型完整版本史 v1→v8；固定轨道库包含定案的全宽 2.0 mm 下方抵墙承压面，J 主体生成器则已迁入独立辅助库。
...
- EN: `wallboard-rail-mount` skill — repository-canonical reusable workflow at `.agents/skills/wallboard-rail-mount/`, including byte-identical rail/J-helper libraries, the 2.0 mm contact contract, Docker verification, and a runnable example.
  ZH: `wallboard-rail-mount` skill —— 仓库内权威可复用工作流，位于 `.agents/skills/wallboard-rail-mount/`，包含字节一致的轨道/J 辅助库、2.0 mm 接触约定、Docker 验证流程及可运行示例。
