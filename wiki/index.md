...
- EN: [hook-scad-v1](outputs/hook-scad-v1.md) — the OpenSCAD model (`hook.scad`, now v3): datum, parameter table, verification, print notes; v3 adds measured D7 (9.0), the top cap, and the wall pad (corrected step direction).
  ZH: [hook-scad-v1](outputs/hook-scad-v1.md) —— OpenSCAD 模型（`hook.scad`，现为 v3）：基准、参数表、验证、打印备注；v3 加入实测 D7（9.0）、顶部小帽、抵墙凸台（纠正台阶方向）。
