...
- EN: v3.1 addendum: fit success on the real rail confirmed by user photos; `hook_w` default 8 → 11.9 (user caliper; worst-case shank stress drops to ≈0.392 MPa/N). At that time the mount interface was extracted to `.agents/skills/wallboard-rail-mount/`; v7 later made the repository `.agents/skills/` copy canonical.
  ZH: v3.1 补充：用户照片确认真实挂条试装成功；`hook_w` 默认 8 → 11.9（用户卡尺实测；杆身最坏应力降至 ≈0.392 MPa/N）。当时安装接口提炼到 `.agents/skills/wallboard-rail-mount/`；v7 后改由仓库 `.agents/skills/` 副本作为权威版本。
